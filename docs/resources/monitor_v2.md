...
- `max_alerts_per_hour` (Number) Sets the maximum number of alerts generated in a single hour before the monitor is deactivated for safety. Must be non-negative. If unset, defaults to 100 (note that we use -1 in the Terraform state to indicate null/unset due to Terraform limitations). When set to 0, the system maximum will be used (typically 3600, i.e. one alert per second). Contact support if you need a higher limit.
