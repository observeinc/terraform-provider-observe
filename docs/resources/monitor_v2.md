...
- `max_alerts_per_hour` (Number) Overrides the default value of max alerts generated in a single hour before the monitor is deactivated for safety. Must be a positive integer (> 0). If unset, the server-side default is used. The upper limit is enforced via layered settings on the Observe side.
