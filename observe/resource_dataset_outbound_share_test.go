		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					// change_tracking should be false by default when not specified
					resource.TestCheckResourceAttr("observe_dataset_outbound_share.test", "change_tracking", "false"),
				),
			},
			{
				Config: fmt.Sprintf(configPreamble+datastreamConfigPreamble+`
					...
					resource "observe_dataset_outbound_share" "test" {
						...
						dataset        = observe_dataset.test.oid
						outbound_share = observe_snowflake_outbound_share.test.oid
						schema_name    = "%[1]s_updated"
						view_name			 = "%[1]s_updated"
						freshness_goal = "30m"
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet("observe_dataset_outbound_share.test", "workspace"),
					resource.TestCheckResourceAttrSet("observe_dataset_outbound_share.test", "oid"),
					resource.TestCheckResourceAttr("observe_dataset_outbound_share.test", "name", randomPrefix+"-updated"),
					resource.TestCheckResourceAttr("observe_dataset_outbound_share.test", "description", "updated test description"),
					resource.TestCheckResourceAttrSet("observe_dataset_outbound_share.test", "dataset"),
					resource.TestCheckResourceAttrPair("observe_dataset_outbound_share.test", "outbound_share", "observe_snowflake_outbound_share.test", "oid"),
					resource.TestCheckResourceAttr("observe_dataset_outbound_share.test", "schema_name", randomPrefix+"_updated"),
					resource.TestCheckResourceAttr("observe_dataset_outbound_share.test", "view_name", randomPrefix+"_updated"),
					resource.TestCheckResourceAttr("observe_dataset_outbound_share.test", "freshness_goal", "30m0s"),
					resource.TestCheckResourceAttr("observe_dataset_outbound_share.test", "change_tracking", "true"),
				),
			},
		},
	})
